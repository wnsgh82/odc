...
public abstract class MyUploadServlet extends HttpServlet{ //추상클래스 
	...
	protected Map<String, String> doFileUpload(Part p, String pathname) throws ServletException, IOException {
		...
		try {
			...
			if(! f.exists()) { // 폴더가 존재하지 않으면 -> 폴더 만들어
				f.mkdirs();
			}
			
			String originalFilename=getOriginalFilename(p);
			if(originalFilename==null || originalFilename.length()==0) return null; //map값이 null이 되면 파일 업로드가 안된거
...
*/
			...
			if(listOriginal.size() != 0) { //단일과의 차이 : 배열로 넘김
	...
	private String getOriginalFilename(Part p) { //클라이언트가 올린 파일이름 -> 확장자 뽑아오기
		try {
